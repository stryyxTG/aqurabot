...
Telegram-бот магазина Telegram-аккаунтов на `aiogram 3`, `Telethon` и SQLite.
...
- Показывает каталог аккаунтов по странам и разделам.
- Продаёт Telegram-аккаунты из `.session`-файлов.
...
- Позволяет админу добавлять аккаунты:
...
- Поддерживает ручные заявки на пополнение через российскую и украинскую карту.
- Имеет команду `/clean`, которая чистит с сервера сессии только проданных товаров и пытается выйти с этих аккаунтов.
...
То есть сейчас включены российская карта, украинская карта и CryptoPay. Other и продажа Premium/Stars отключены в UI/handlers.
...
- `/clean` — очистить с сервера только проданные товары и выйти с их аккаунтов.
...
- `products` — товары/аккаунты, session path, телефон, 2FA, статус.
...
- Для добавления аккаунтов используется `API_ID`/`API_HASH` из конфига.
...
- Получение кода из `777000` может найти не тот код, если в аккаунте несколько свежих кодов. Это известная зона риска.
...
- не должен чистить непроданные аккаунты.
...
- Нужно изменить добавление/проверку аккаунтов — `shopbot/session_flow.py`.
...
- Логи могут содержать чувствительные данные, особенно ответы внешних API и данные аккаунтов.
